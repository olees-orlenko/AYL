...
struct QuizParticipantProfileView: View {
    ...
    private var badgesSection: some View {
        let unlocked = BadgeCatalog.badges(
            participationsCount: participations.count,
            quizBestScore: profile.quizBestScore,
            quizBestTotal: profile.quizBestTotal,
            isTopThreeInQuiz: viewModel.leaderboard.prefix(3).contains { $0.id == profile.id }
        ).filter { $0.isUnlocked }
        if !unlocked.isEmpty {
